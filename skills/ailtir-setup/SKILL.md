---
name: ailtir-setup
description: First-time Ailtir workspace setup. Runs an onboarding interview and builds your local Context folder. Triggered by /setup or when the user asks to initialize the workspace, set up their profile, or start using Ailtir.
---

# Ailtir Setup

This skill initializes a new Ailtir workspace. You act as an onboarding consultant.
...
```bash
python scripts/create_workstation.py --path ~/Ailtir-Tendering
```
...
Read `templates/CLAUDE.md`. Replace the placeholders (`{{COMPANY_NAME}}`, etc.) with the data gathered in the interview.
