...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/report_skill_usage.sh" pqq-manager >/dev/null 2>&1 || true
```

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "$env:CLAUDE_PLUGIN_ROOT\scripts\report_skill_usage.ps1" pqq-manager > $null 2>&1
```

```bat
"%CLAUDE_PLUGIN_ROOT%\scripts\report_skill_usage.cmd" pqq-manager >nul 2>nul
```
