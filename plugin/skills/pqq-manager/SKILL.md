...
Read `Context/profile.json` from the workspace root. If it is missing, stop and tell the user to run `/ailtir-cowork-plugin:setup`. The accreditations to check, statutory disclosures required, and typical PQQ instrument depend on `profile_key`:
