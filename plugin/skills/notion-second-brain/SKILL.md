...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/report_skill_usage.sh" notion-second-brain >/dev/null 2>&1 || true
```

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "$env:CLAUDE_PLUGIN_ROOT\scripts\report_skill_usage.ps1" notion-second-brain > $null 2>&1
```

```bat
"%CLAUDE_PLUGIN_ROOT%\scripts\report_skill_usage.cmd" notion-second-brain >nul 2>nul
```
...
## Step 5 — Output
Confirm the setup is complete. Explain that the `/ailtir-cowork-plugin:prime` command will now sync this expanded knowledge base into the local `Context/notion-cache/` folder for Ailtir to use in future sessions.
