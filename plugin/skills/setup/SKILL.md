...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/report_skill_usage.sh" setup >/dev/null 2>&1 || true
```

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "$env:CLAUDE_PLUGIN_ROOT\scripts\report_skill_usage.ps1" setup > $null 2>&1
```

```bat
"%CLAUDE_PLUGIN_ROOT%\scripts\report_skill_usage.cmd" setup >nul 2>nul
```
...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/run_python.sh" "${CLAUDE_PLUGIN_ROOT}/skills/setup/scripts/create_workstation.py"
```
...
Resolve the workspace root from `AILTIR_PLUGIN_DATA`, or use `~/Ailtir-Tendering` if it is not set. Use the file operation tools to copy and customise the templates bundled with this skill:

- Read `${CLAUDE_PLUGIN_ROOT}/skills/setup/templates/Context/company.md`, replace placeholders, and write to `Context/company.md`.
- Read `${CLAUDE_PLUGIN_ROOT}/skills/setup/templates/Context/team.md`, replace placeholders, and write to `Context/team.md`.
- Read `${CLAUDE_PLUGIN_ROOT}/skills/setup/templates/Context/connectors.md`, replace `{{ACTIVE_CONNECTORS}}` with the user's Notion status, and write to `Context/connectors.md`.

## Step 4 — Write the Global CLAUDE.md

Read `${CLAUDE_PLUGIN_ROOT}/skills/setup/templates/CLAUDE.md`. Replace the placeholders (`{{COMPANY_NAME}}`, `{{LOCATION}}`, `{{SECTORS}}`, `{{ACCREDITATIONS}}`, `{{DATE}}`) with the data gathered in the interview.
