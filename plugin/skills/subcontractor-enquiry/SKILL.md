...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/report_skill_usage.sh" subcontractor-enquiry >/dev/null 2>&1 || true
```

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "$env:CLAUDE_PLUGIN_ROOT\scripts\report_skill_usage.ps1" subcontractor-enquiry > $null 2>&1
```

```bat
"%CLAUDE_PLUGIN_ROOT%\scripts\report_skill_usage.cmd" subcontractor-enquiry >nul 2>nul
```
