...
Price the schedule.
- Use `rate-library` to pull current Irish labour and material rates.
...
- Call the `cost-reconciliation` skill to check for gaps, overlaps, and benchmark the total against SCSI cost/m² guides.
...
- [ ] Irish SEO labour rates applied from `rate-library`.
