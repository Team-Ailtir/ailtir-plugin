---
name: ailtir_kb_chat
description: Ask a natural-language question against an Ailtir knowledge base. Invoke with /ailtir:ailtir_kb_chat <kb_id> <question>. Sends the last 5 conversation interactions as context alongside the question.
...
2. **If either is missing**, ask the user to provide both the `kb_id` and the
   question before proceeding. Offer to run `/ailtir:ailtir_kb_list` to find a valid `kb_id`.
